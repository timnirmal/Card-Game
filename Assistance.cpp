...
void display_without_symbols(int n, card cards[]){
    for(int i=0; i<5;i++){
        int card_out_val = n/10;
        int card_out_suit = n-((n/10)*10);

        if(cards[i].getValue()==card_out_val && cards[i].getSuit()==card_out_suit){
            cards[i].display_card_without_symbols();
        }
    }
}
...
Assistance::Assistance() {
    ...
    wcout<<endl<<"Cards in symbol less format : ";
    display_without_symbols(non_hidden_card, cards); wcout<<" ";
    display_without_symbols(used_cards[0], cards); wcout<<" ";
    display_without_symbols(used_cards[1], cards); wcout<<" ";
    display_without_symbols(used_cards[2],cards); wcout<<" ";
